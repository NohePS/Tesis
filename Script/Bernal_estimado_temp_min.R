...
model_Btmin_1 <- lm(Bernal_temp_min  ~    miraflores_temp_min+ chusis_temp_min,
                     data = data_temp_min)#0.9267

model_Btmin_2 <- lm(Bernal_temp_min  ~    Esperanza_temp_min+
                      chusis_temp_min,
                    data = data_temp_min)#0.9154
model_Btmin_3 <- lm(Bernal_temp_min  ~    mallares_temp_min+
                      chusis_temp_min,
                    data = data_temp_min) #0.9167
model_Btmin_4 <- lm(Bernal_temp_min  ~    miguel_temp_min+ 
                      chusis_temp_min,
                    data = data_temp_min) #0.9254
model_Btmin_5 <- lm(Bernal_temp_min  ~    UDEP_temp_min+ 
                     mallares_temp_min,
                    data = data_temp_min) #0.8675
model_Btmin_6 <- lm(Bernal_temp_min  ~    miraflores_temp_min+ 
                      miguel_temp_min,
                    data = data_temp_min)#0.9016
model_Btmin_7 <- lm(Bernal_temp_min  ~    Esperanza_temp_min+ 
                      mallares_temp_min,
                    data = data_temp_min)#0.8741
model_Btmin_8 <- lm(Bernal_temp_min  ~    UDEP_temp_min+ 
                      Esperanza_temp_min,
                    data = data_temp_min)#0.8626
model_Btmin_9 <- lm(Bernal_temp_min  ~    miguel_temp_min+ 
                      Esperanza_temp_min,
                    data = data_temp_min)#0.8961
model_Btmin_10 <- lm(Bernal_temp_min  ~    miguel_temp_min+ 
                      mallares_temp_min,
...
for (j in 1971:2019){
  ...
  for (i in 1:length(Bernal_bymonth$year)){
    ...
    if(Bernal_bymonth$year[i] == j){
      ...
      if(is.na(Bernal_bymonth$min_temp_min[i]) == TRUE){
        cont = cont +1
      }else {
        cont=cont
      }
